...
/** Cache. Values are seconds, matched to the 5 minute refresh below. */
const CACHE_DIR   = __DIR__ . '/.airthings-cache';
const TTL_LATEST  = 300;
const TTL_META    = 900;
...
/** How often the page pulls new readings, in seconds. Sensors report every 2.5 to 5 minutes. */
...
/**
 * Floor mapping. Airthings has no floor field, so floors are read from room names
 * and anything below overrides that. Keyed by device serial number, so a room can
 * be renamed without breaking it. Floors set in the browser take precedence again,
 * and are stored in .airthings-floors.json beside this file.
 */
const FLOOR_OVERRIDES = [
    // Level 13
    '2969003664' => 'Level 13',   // Flyover
    '2969002937' => 'Level 13',   // Space One
    '2969002729' => 'Level 13',   // Space Two
    '2969003652' => 'Level 13',   // Space Three
    '2969003857' => 'Level 13',   // The Axis

    // Level 12
    '2969002771' => 'Level 12',   // Space Four
    '2969003805' => 'Level 12',   // Space Five
    '2969003780' => 'Level 12',   // Space Six
    '2969002703' => 'Level 12',   // Space Seven
    '2969003655' => 'Level 12',   // Space Eight
    '2969003499' => 'Level 12',   // Space Nine
    '2969003731' => 'Level 12',   // Wolfson Lecture Theatre
    '2969003639' => 'Level 12',   // Wolfson Virtual Theatre

    // Level 11
    '2969002724' => 'Level 11',   // Assessment Prep Room
    '2969002710' => 'Level 11',   // Dinwoodie (inside Assessment Room)
    '2969002889' => 'Level 11',   // Jerwood simulation
    '2969003815' => 'Level 11',   // Patient holding
    '2969002728' => 'Level 11',   // Space 11/Examiner holding

    // Level 10
    '2969002772' => 'Level 10',   // Outside meeting rooms
    '2969002769' => 'Level 10',   // UOL outside small office
    '2969002883' => 'Level 10',   // Workspace - Side B (near printer)

    // Level 1
    '2969002659' => 'Level 1',    // Reception Desk (facing Novotel)

    // Ground floor
    '2969002908' => 'Ground floor', // Reception
];
...
function fetch_latest(bool $fresh = false): array
{
    ...
    foreach ($meta['locations'] as $loc) {
        ...
        foreach (($res['devices'] ?? []) as $dev) {
            ...
            $override = (string)($floorMap[$serial] ?? (FLOOR_OVERRIDES[$serial] ?? ''));
            if ($override !== '') {
                $floor = $override;
...
async function loadLatest(fresh) {
  ...
  try {
    ...
    state.refresh = data.refresh || 300;
    ...
    kioskAlert("No data from Airthings: " + err.message + " Retrying in " +
      Math.round((state.refresh || 300) / 60) + " minutes.");
  }
}
...
function tick() {
  ...
  el("clockNote").textContent = state.countdown > 90
    ? "Next refresh in " + Math.ceil(state.countdown / 60) + " min"
